...
class CrimeCctvModel():
    ...
    def sum_pol(self):
        ...
        re_police = re_police.groupby('구별').sum()
        # re_police.drop(['관서명'], axis=1, inplace=True)
        printer.dframe(re_police)
        re_police.to_csv(vo.context+'sum_police_positions.csv', index=False)
        return re_police

    def compression_police(self):
        vo = self.vo
        reader = self.reader
        printer = self.printer
        vo.context = 'admin/crime/data/new_data/'
        vo.fname = 'police_positions'
        comp_police = reader.csv(reader.new_file(vo))
        _ = self.crime_columns
        comp_police['범죄 발생'] = comp_police[_[0]] + comp_police[_[1]] + comp_police[_[2]] + comp_police[_[3]]
        _ = self.arrest_columns
        comp_police['범죄 검거'] = comp_police[_[0]] + comp_police[_[1]] + comp_police[_[2]] + comp_police[_[3]]
        # comp_police.loc[['구별', '범죄발생', '범죄 검거']]
        # comp_police['범죄 검거'] = comp_police.groupby([self.arrest_columns]).apply(lambda row: '+'.join(row.values.astype(str)), axis=1)
        printer.dframe(comp_police)
