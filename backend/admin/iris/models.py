...
class Iris(object):
    ...
    def iris_by_tf(self):
        ...
        # print(f'type: {train_dataset_fp}')  # 해당 경로로 가서 data 폴더로 옮겨준다.
        ...
        features, labels = next(iter(train_dataset))
        print(f'train_dataset_features[:5] 의 값 : {features[:5]}')
        test_dataset = test_dataset.map(self.pack_features_vector)
        features, labels = next(iter(test_dataset))
        print(f'test_dataset_features[:5] 의 값 : {features[:5]}')
        '''
        모델 타입 선정
        ...
        * 훈련 데이터세트에 대해 너무 많이 배우면 예측이 관측한 데이터에 대해서만 작동하고
         일반화할 수 없습니다. 이런 문제를 과대적합(Over fitting)이라고 하며, 
        ...
        # plt.savefig(f'{self.vo.context}train_accuracy_results')
        '''
    ...
    def base(self):
        ...
        # print(f'품종이 아이리스 데이터 구조: {iris_df.head(2)}\n{iris_df.columns}')
        '''
    ...
    '''
    # error/loss 의 결정 경계선
    def draw_decision_regions(self, X, y, classifier, resolution):
        # 마커와 컬러맵을 설정합니다
        markers = ('s', 'x', 'o', '^', 'v')
