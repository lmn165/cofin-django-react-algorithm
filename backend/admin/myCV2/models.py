...
class MyCV2(object):
    ...
    def face_detect(self):
        ...
        cv2.imwrite(f'{vo.context}new_data/face_detection.png', image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    ...
    def lena(self):
        ...
        cv2.imwrite(f'{vo.context}new_data/lena_original.png', original)
        cv2.imwrite(f'{vo.context}new_data/lena_gray.png', gray)
        cv2.imwrite(f'{vo.context}new_data/lena_unchanged.png', unchanged)
    ...
    def girl(self):
        ...
        cv2.imwrite(f'{vo.context}new_data/girl_negative.png', negative)
        bgr2gray = cv2.cvtColor(original, cv2.COLOR_BGR2GRAY)
        th = 90 # 역치 지정
        bgr2gray[bgr2gray > th] = 255
        bgr2gray[bgr2gray < th] = 0
        cv2.imwrite(f'{vo.context}new_data/girl_cvtColor.png', bgr2gray)
        small_image = original[150:450, 150:450]
        small_image = cv2.resize(small_image, (400,400))
        cv2.imwrite(f'{vo.context}new_data/girl_small_image.png', small_image)
        cv2.waitKey(0)  # 키입력을 기다리는 대기함수, 0은 즉시 실행
        cv2.destroyAllWindows()  # 윈도우 종료
